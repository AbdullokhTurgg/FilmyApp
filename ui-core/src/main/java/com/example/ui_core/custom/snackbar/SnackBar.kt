...
import com.example.ui_core.databinding.LayoutGenericSnackbarBinding
import com.example.ui_core.databinding.LayoutGenericSnackbarTimerBinding
import com.google.android.material.snackbar.BaseTransientBottomBar
import com.google.android.material.snackbar.Snackbar
import com.example.ui_core.extensions.extensions.layoutInflater
...
open class SnackBar private constructor() {
    ...
    open class Builder(val rootView: View) {
        ...
        open fun build() = assembleSnackbar(message = message,
    ...
    companion object {
        ...
        private fun assembleSnackbar(
            ...
            val binding = LayoutGenericSnackbarBinding.inflate(rootView.context.layoutInflater())

            val snackbar = inflateSnackBar(customLayout = binding.root,
                rootView = rootView,
                duration = duration)
            setSnackbarText(text = message, textView = binding.message)
            setSnackbarButtonText(textView = binding.button,
                divider = binding.divider,
                text = buttonText)
            setSnackbarImage(imageView = binding.icon,
                container = binding.iconContainer,
                drawable = icon)
            setSnackbarButtonClickListener(textView = binding.button,
                listener = listener,
                snackbar = snackbar)
            return snackbar
        }
...
//            binding.progressBar.progress = Int.MAX_VALUE
            ...
            val genericSnackbarBaseCallback =
                GenericSnackbarBaseCallbackImpl(progressBar = binding.progressBar,
                    duration = duration,
        ...
        private fun inflateSnackBar(customLayout: View, rootView: View, duration: Int) =
            GenericSnackbarInflter.Base()
                .inflate(rootView = rootView, customLayout = customLayout, duration = duration)
    ...
    class TimerSnackbarBuilder(rootView: View) : Builder(rootView) {
        ...
        override fun build() = assembleSnackbar(message = message,
            buttonText = buttonText,
            listener = buttonClickListener,
            timeOverListener = timeOverListener,
